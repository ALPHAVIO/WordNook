...
router.get(
  ...
  async function (req, res) {
    ...
    if (!loggedUser) {
      ...
      await Blog.find({}, function (err, posts) {
        if (!err) {
          posts.forEach(function (post) {
            ...
            if (storedTitle === requestedTitle) {
              ...
              console.log("He is isAuthor ", isAuthor);

              ////number of views get increased by one
              if (isAuthor == false) {
                Blog.findOne(
                  { blogTitle: req.params.postName },
                  function (err, doc) {
                    if (err) console.log(err);
                    doc.noOfViews += 1;
                    console.log(doc.noOfviews);
                    doc.save(function (err) {
                      if (err) console.log(err);
                    });
                    console.log("This person is ", req.user);
                    console.log("Author is ", doc.author);
                  }
                );
              }
            }
...
router.post(
  "/posts/:postName/comments/:commentNum",
  auth,
  async function (req, res) {
...
router.post("/posts/:postName", auth, (req, res, next) => {
  ...
  Blog.deleteOne({ blogTitle: requestedTitle, author: user._id })
    .then(() => {
      res.redirect("/");
    })
    .catch((error) => {
